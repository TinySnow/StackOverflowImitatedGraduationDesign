...
public class LoginController {
    ...
    public Response body(@RequestBody LoginBody loginBody) {
        // 生成令牌
        String token = loginService.login(loginBody.getEmail(), loginBody.getPassword())[0];
        String userId = loginService.login(loginBody.getEmail(), loginBody.getPassword())[1];
        if (token == null) {
        ...
        response.put(CommonConstants.JWT_USERID, userId);
        return response;
    }
}
